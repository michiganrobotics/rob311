...
class MPU6050:
    ...
    CONST_G                       = 9.81 # gravitational const
    ...
    def initialize(self):
        self.setClockSource(self.MPU6050_CLOCK_INTERNAL)
    ...
    def dmpInitialize(self):
        ...
        self.setRate(0) # 1khz / (1 + 4) = 200 Hz [9 = 100 Hz]
    ...
    def getAcc(self):
        ...
        for i in range(59,64,2):
            val = (self.i2c.readBit( 0x68, i)<<8)+self.i2c.readBit( 0x68,i+1)
            if val > 65535/2:
                val = -1*(65535 - val)
            output.append((val*4*9.806)/65535.0) #Normalize Unit for +-2g
        #print "modified Acc Val X: "+str(output[0])
        return output
    ...
    def getAccYZ(self):
        for i in xrange(3):
            try:
                output = []
                for i in range(61,64,2):
                    val = self.i2c.bus.read_i2c_block_data(self.MPU6050_DEFAULT_ADDRESS,i,2)
                    val = (val[0]<<8)+val[1]
                    if val > 65535/2:
                        val = -1*(65535 - val)
                    output.append((val*4*self.CONST_G)/65535.0) #Normalize Unit for +-2g
                return output
            except:
                pass
                #print 'MPU Acc i2c  error...'
                print traceback.format_exc()
        print 'MPU Acc i2c  error... - failed 3 times'
        return [-1.0, -1.0]
    ...
    def getGyroX(self):
        for i in xrange(3):
            try:
                output = []
                val = self.i2c.bus.read_i2c_block_data(self.MPU6050_DEFAULT_ADDRESS,67,2)
                val = (val[0]<<8)+val[1]
                if val > 65535/2:
                    val = -1*(65535 - val)
                output.append(math.radians((val *2* 250)/65535.0))#Normalize Unit for +-250degrees/second, convert to radians
                return output
            except:
                pass
                #print 'MPU Gyro i2c  error...'
                print traceback.format_exc()
        print 'MPU Gyro i2c  error... - failed 3 times'
        return [-1.0]
